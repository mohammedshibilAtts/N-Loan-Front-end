...
import { useIncomeExpenses } from "./useIncomeExpenses";
...
export default function IncomeExpensesReport() {
    ...
    const { fetchIncomeExpenseReport, reportData: apiReportData, loading } = useIncomeExpenses();
    ...
    useEffect(() => {
        ...
        fetchIncomeExpenseReport(payload);
    }, [dateRange]);

    useEffect(() => {
        if (apiReportData) {
            const data: ReportData = apiReportData;
            setReportData(data);
            ...
            if (incomeBreakdown) {
                Object.keys(incomeBreakdown).forEach(key => {
                    ...
                    if (key === 'otherIncomeBreakdown' && Array.isArray(val)) {
                        val.forEach((item: any) => {
                            labels.push(item.name);
                            incomeValues.push(item.amount);
                            expenseValues.push(0);
                        });
                        return;
                    }

                    if (key === 'otherIncome' && Array.isArray(incomeBreakdown['otherIncomeBreakdown']) && incomeBreakdown['otherIncomeBreakdown'].length > 0) {
                        return;
                    }
