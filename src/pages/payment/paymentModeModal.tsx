...
const PaymentModal: React.FC<PaymentModalProps> = ({
    ...
    const mappedData = entries.map((item: any, index: number) => ({
        id: index + 1,
        paymentMethod: paymentModes.find((m: any) => m._id === item.paymentMethod)?.mode || "Loading...",
        paymentProvider: paymentProviders.find((p: any) => p._id === item.paymentProvider)?.providerName || (item.paymentProvider ? "Loading..." : "N/A"),
            ...
            <form onSubmit={formik.handleSubmit}>
                <DialogContent>
                    <Grid container spacing={2}>
                        ...
                        {(() => {
                            const mode = paymentModes.find((p: any) => p._id == selectedMethod);
                            if (mode?.mode != "Cash") {
                                return (
                                    <Grid item xs={12} sm={12} md={4}>
